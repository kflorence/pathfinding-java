...
public final class AStar
{
    ...
    public Node get(int n)
    {
    	...
    	return pathList.get(getSize() - n);
    }
    
    // Returns Cell from Node at location n on pathList
    public Cell getMove(int n)
    {
    	return get(n).getCell();
    }
       	    ...
       	    if (entity.debugOn())
       	    {
	            // Output the path information for each entity to text files
	           	try
	           	{
		           	...
		           	out.newLine();
		           	out.write("Final path size: " + getSize() + " nodes.");
		           	out.newLine();
    ...
    private ArrayList<Node> buildPath(Node node)
   	{    	 
       	// Create a temporary list for this loop
	   	ArrayList<Node> tempList = new ArrayList<Node>();
	   	
       	// Build our updated pathList
	   	do tempList.add(node);
